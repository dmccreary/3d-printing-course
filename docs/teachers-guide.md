...
description: Comprehensive instructor's guide for the Introduction to 3D Printing intelligent textbook — classroom usage, MicroSims, quizzes, licensing, and customization.
---

# 3D Printing Teacher's Guide

Welcome to the teacher's guide for *Introduction to 3D Printing: An Interactive Intelligent Textbook with MicroSims*. This guide explains every feature of the textbook, how to use it in your classroom, and how to customize it for your students. No prior technical knowledge is assumed — every technical term is defined before it is used.
...
- **Critical thinking emphasis** — every chapter helps students evaluate 3D printing technologies and workflow choices against real engineering specifications, and distinguish evidence-based technical guidance from vendor marketing
- **College-credit pathway framing** — content is aligned with America Makes workforce competencies, ASTM/ISO 52900 standards, and PLTW engineering pedagogy so students can pursue dual credit and articulation at community colleges
- **Learning graph** — a visual map showing how all 292 concepts connect and build on each other
- **Benchy** — a friendly mascot character (called a "pedagogical agent") who guides students through each chapter with tips, encouragement, and key insights
...
| Chapters | Topic Area |
|----------|-----------|
| 1–2 | Foundations (history of additive manufacturing, ISO/ASTM 52900 standards and the seven process families) |
| 3–5 | Design and Digital Workflow (PLTW engineering design process, parametric CAD, file formats and mesh repair) |
| 6–7 | Materials and Slicing (materials science for AM, slicing and toolpath generation) |
| 8–10 | Printer Technology (FDM hardware and operation, safety/ethics/sustainability, resin printing) |
| 11–12 | Quality and Problem-Solving (Design for Additive Manufacturing and metrology, troubleshooting and post-processing) |
| 13–14 | Industry Context (modern hardware landscape, modern AM ecosystem) |
| 15–16 | Advanced Topics and Careers (AI and machine learning in AM, digital workflows and capstone project) |
...
5. **Welcome from Benchy** — A mascot admonition that introduces the chapter topic in Benchy's friendly voice.
6. **Main content** — The core instructional material, written at a Grades 10–12 reading level. Includes tables, real-world examples, and embedded MicroSims.
7. **Mascot admonitions** — Throughout the chapter, Benchy appears 5–6 times to highlight key insights (thinking), offer practical tips (tip), provide encouragement on harder concepts (encouraging), and warn about common mistakes (warning).
8. **Key takeaways** — A numbered summary of the most important concepts, preceded by a celebration from Benchy.
...
- **After class**: Assign the practice questions. Use the quiz page for a quick formative assessment.
- **Pacing**: Each chapter is designed for approximately 2–3 class periods (90–135 minutes of instruction). Chapters with more hands-on printer operation content (Chapters 7, 8, 10, 12) may need additional lab time.
...
Each MicroSim lets students manipulate one or more variables (using sliders, buttons, or drag-and-drop) and immediately see how the model responds. This "learn by doing" approach helps students build intuition for abstract concepts like how layer height affects print time, or how infill density relates to part strength.
...
The textbook includes MicroSims built with different visualization technologies:

| Technology | What It's Good For | Example Use |
|-----------|-------------------|-------------|
| **p5.js** | Interactive animations with sliders and buttons | Layer-height and print-time trade-off explorer |
| **Chart.js** | Bar charts, line charts, pie charts | Material property comparisons |
| **Plotly** | Advanced interactive charts with hover details | Slicer parameter effect visualization |
...
!!! mascot-tip "Benchy's Tip: Embed MicroSims Anywhere!"
    <img src="img/mascot/tip.png" class="mascot-admonition-img" alt="Benchy shares a tip">
    ...
    ```html
    <iframe src="https://dmccreary.github.io/3d-printing-course/sims/YOUR-MICROSIM-NAME/main.html"
...
The **glossary** is an alphabetical list of 292 key terms used in the textbook, each with a precise, concise definition. It serves as a quick-reference dictionary for students encountering unfamiliar vocabulary — everything from "anisotropy" to "vat photopolymerization."
...
The **FAQ** (Frequently Asked Questions) is a curated list of common questions students ask about 3D printing and additive manufacturing, organized by topic. Each question includes a clear, concise answer written at the same reading level as the chapters.

### How the FAQ is Organized

The FAQ covers questions across all 16 chapters. Questions are grouped by topic area (foundations, design workflow, materials, slicing, printer operation, troubleshooting, careers) to make browsing easy.
...
A well-balanced assessment includes questions across multiple levels. The quizzes in this textbook primarily target levels L1–L4, with the capstone project in Chapter 16 targeting L5–L6.

## Using the References

### What Are the References?

Each chapter has an accompanying **references page** with a curated list of approximately 10 high-quality sources that students can use for further reading. References prioritize Wikipedia articles for accessibility and reliability, supplemented by authoritative sources from ASTM International, America Makes, PLTW, and peer-reviewed engineering literature.
...
1. Go to the textbook's GitHub repository: [dmccreary/3d-printing-course](https://github.com/dmccreary/3d-printing-course)
2. Click the **"Issues"** tab at the top of the page
3. Click the green **"New issue"** button
4. Give your issue a clear title (e.g., "Broken link in Chapter 5 references" or "Suggestion: Add MicroSim for slicer temperature")
...
1. Go to [dmccreary/3d-printing-course](https://github.com/dmccreary/3d-printing-course)
...
Replace the file `docs/img/logo.png` with your own logo image (PNG format, approximately 128×128 pixels).
...
4. Open your browser to `http://127.0.0.1:8000/3d-printing-course/` to see your customized version
...
This command builds the website and publishes it to `https://YOUR-USERNAME.github.io/3d-printing-course/`. The process takes about 1–2 minutes.
...
**Recommendation**: The Google Analytics setup described above is anonymous by default — it tracks aggregate page views, not individual students. This is the safest approach for a K–12 classroom. If you want individual student tracking via xAPI, consult your school district's data privacy officer before proceeding.
...
For example, understanding "DfAM overhang rules" requires mastering "support structures" and "layer adhesion" first. The learning graph makes these dependency chains visible.
...
The interactive Learning Graph Viewer is available under **MicroSims → Learning Graph Viewer** in the left navigation.

## Benchy: Your Pedagogical Agent
...
Benchy is an anthropomorphic 3D-printing tugboat — inspired by the classic 3DBenchy calibration model used by the 3D printing community worldwide. He appears as colored callout boxes (called **admonitions**) throughout each chapter. There are seven types:
...
| Encouraging | Blue | Supports on harder concepts | Where students may struggle |
| Celebration | Purple | Celebrates progress | Every chapter ending |
| Neutral | Gray | General notes | As needed |

Benchy appears no more than 5–6 times per chapter to avoid overuse. Mascot admonitions are never placed back-to-back.

### Tips for Teachers

- **Read Benchy's tips aloud** — They're written in a conversational tone that works well when spoken
- **Use as discussion prompts** — Benchy's "thinking" admonitions highlight the most important insights in each chapter
- **Encourage struggling students** — Point students to Benchy's "encouraging" admonitions when they're frustrated with a concept
- **Benchy's catchphrase** — "Let's make something great!" — is a useful classroom rallying cry before lab sessions
